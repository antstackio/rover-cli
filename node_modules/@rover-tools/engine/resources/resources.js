...
exports.resourceGeneration = exports.apigatewaypath = exports.skeleton = void 0;
var configs = require("../utlities/config");
var utilities = require("../utlities/utilities");
...
function rolePolicyAddition(template, config) {
    var policies = [];
    var base = template["Properties"]["ManagedPolicyArns"][0];
    ...
    for (var k in config["Policies"]) {
        var role = JSON.parse(JSON.stringify(configs.PolicySkeleton));
...
function swaggerGenerator(config) {
    var swagger = JSON.parse(JSON.stringify(configs.SwaggerSkeleton));
    var swaggerPaths = {};
    if (config.hasOwnProperty("security")) {
        var securities = Object.keys(JSON.parse(JSON.stringify(config["security"])));
    ...
    config.objects.map(function (data, i) {
...
var attachMethods = function (methodArray, data, security) {
    var result = {};
    if (methodArray.length) {
        methodArray.map(function (item) {
            result[item] = JSON.parse(JSON.stringify(configs.SwaggerPathSkeleton[item]));
            if (item !== "options") {
                var uri = result[item]["x-amazon-apigateway-integration"]["uri"]["Fn::Sub"] + configs.APIGatewayURI[data["resourcetype"]];
...
function apigatewaypath(template, path) {
    var definationbody = JSON.parse(JSON.stringify(configs.APIGatewaySkeleton));
...
var resourceGeneration = function (resource_name, config) {
    var resource_properties = JSON.parse(JSON.stringify(configs.AWSResources[resource_name]));
    var template = {};
    for (var j in resource_properties.attributes) {
        if (resource_properties.attributes[j] == "Type") {
            ...
            if (config["DependsOn"] !== undefined)
                template[resource_properties.attributes[j]] = JSON.parse(JSON.stringify(config["DependsOn"]));
        }
        else {
